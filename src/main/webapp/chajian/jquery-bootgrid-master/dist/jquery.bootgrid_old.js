...
;(function ($, window, undefined)
	...
	function renderRows(rows)
	{
		if (rows.length > 0)
		{
			...
			$.each(rows, function (index, row)
				...
				var status = row!=null&&row.status != null && that.options.statusMapping[row.status];
				if (status)
				{
					rowCss += status;
				}
	...
	Grid.defaults = {
			...
			labels: {
				all: "All",
				infos: "展示 {{ctx.start}} - {{ctx.end}} 共{{ctx.total}} 条",
				loading: "加载...",
				noResults: "没有数据!",
				refresh: "刷新",
				search: "搜索"
			...
			templates: {
				...
				header: "<div id=\"{{ctx.id}}\" class=\"{{css.header}}\"><div class=\"row\"><div id=\"buttons_id\" class=\"col-sm-7\"></div><div class=\"col-sm-5 actionBar\"><p class=\"{{css.search}}\"></p><p class=\"{{css.actions}}\"></p></div></div></div>",
