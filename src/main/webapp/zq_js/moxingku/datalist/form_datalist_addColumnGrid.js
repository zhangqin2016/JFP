...
Ext.onReady(function() {
	...
	store = Ext.create('Ext.data.Store', {
		...
		proxy : {
			type : 'ajax',
			url : ctx + '/zq/dataList/column/gridAddJson.zq?dataListId='+dataListId,
			reader : {
				type : 'json',
				root : 'root'
			}
	...
	function add(){
		  var selModel = grid.getSelectionModel();
		    if (selModel.hasSelection()) {
		                ...
		                $.post(ctx+"/zq/dataList/column/add.zq",{dataListId:dataListId,columns:Ext.JSON.encode(obs)},function(data){
		                	alert(data);
		                	parent.store.reload();
		                })
		    }
