...
Ext.onReady(function() {
 ...
 store=Ext.create('Ext.data.Store', {
    model:'NavigationSecond',
    proxy: {
        type: 'ajax',
        url: ctx + '/portal/navication/three/gridJson.zq?second_id='+second_id,
        reader: {
            type: 'json',
            root: 'root'
        }
...
var grid=Ext.create('Ext.grid.Panel', {
    ...
    columns: [
              ...
              { text: 'second_id',  dataIndex: 'secondId',hidden:true },
              { text: 'first_id',  dataIndex: 'firstId',hidden:true },
        { text: '菜单名称',  dataIndex: 'threeName',flex: 0.3, editor: {
            allowBlank: false
        }},
        { text: '菜单地址',  dataIndex: 'threeUrl',flex: 0.3, editor: {
            allowBlank: false
        }},
...
function save(){
    ...
    Ext.Ajax.request({
        url:ctx+'/portal/navication/three/save.zq',
        method: "post",
        params: {
        ...
        success: function (response, opts) {
            ...
            if (response.responseText) {
              	parent.regreshNodeById('second_'+second_id);
             store.reload();
...
function dodel(ids){
    ...
    Ext.MessageBox.show({
        ...
        waitConfig: { interval: 10 }
            ...
            Ext.Ajax.request({
                url:ctx+'/portal/navication/three/del.zq',
                method: "post",
                params: {ids:ids},
                success: function (response, opts) {
                    Ext.MessageBox.hide();
...
function openTab(id,name){
	...
	$(parent.document).find("#liebiao").attr("src",ctx+"/portal/navication/three.zq?metadata_id="+id);
}
