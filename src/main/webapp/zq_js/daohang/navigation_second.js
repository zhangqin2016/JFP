...
Ext.onReady(function() {
 ...
 store=Ext.create('Ext.data.Store', {
    ...
    proxy: {
        type: 'ajax',
        url: ctx + '/portal/navication/second/gridJson.zq?first_id='+first_id,
        reader: {
            type: 'json',
            root: 'root'
        }
...
var grid=Ext.create('Ext.grid.Panel', {
    ...
    columns: [
              ...
              { text: 'firstId',  dataIndex: 'firstId',hidden:true },
        { text: '菜单名称',  dataIndex: 'secondName',flex: 0.3, editor: {
            allowBlank: false
        }},
        { text: '菜单地址',  dataIndex: 'secondUrl',flex: 0.3, editor: {
            allowBlank: false
        }},
...
function save(){
    ...
    Ext.Ajax.request({
        url:ctx+'/portal/navication/second/save.zq',
        method: "post",
        params: {
        ...
        success: function (response, opts) {
            ...
            if (response.responseText) {
              	parent.regreshNodeById('first_'+first_id);
             store.reload();
...
function dodel(ids){
    ...
    Ext.MessageBox.show({
        ...
        waitConfig: { interval: 10 }
            ...
            Ext.Ajax.request({
                url:ctx+'/portal/navication/second/del.zq',
                method: "post",
                params: {ids:ids},
                success: function (response, opts) {
                    Ext.MessageBox.hide();
...
function openTab(id,name){
	...
	$(parent.document).find("#liebiao").attr("src",ctx+"/portal/navication/second/field.zq?metadata_id="+id);
}
